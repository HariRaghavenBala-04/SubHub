...
export default function League() {
  ...
  return (
    <main style={{ padding: '40px 32px', maxWidth: 1100, margin: '0 auto' }}>
      ...
      {loading && (
        <div style={{ color: 'var(--muted)', textAlign: 'center', padding: 60, fontSize: 16 }}>
          Loading teams…
        </div>
      )}
      ...
      {error && (
        ...
        >
          API error: {error}. Check that the backend is running and your API key is set.
        </div>
      )}
      ...
      >
        {teams.map(team => (
          <Link key={team.id} to={`/squad/${team.id}?league=${code}`} style={{ textDecoration: 'none' }}>
            <div
